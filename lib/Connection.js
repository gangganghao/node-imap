...
function Connection(config) {
  ...
  this._idCommand = config.idCommand||{ version: "1.0", name: "ShaoZi", vendor: "ShaoZi"}
  ...
  this._responseOkText = undefined;
}
...
Connection.prototype._resUntagged = function(info) {
  ...
  if (type === 'bye')
    ...
    this._responseOkText = info.text;
    if (this.state === 'connected' && !this._curReq)
      this._login();
...
Connection.prototype._login = function() {
  ...
  var reentry = function(err) {
    ...
    if (!checkedNS && self.serverSupports('NAMESPACE') && !(this._responseOkText||'').toLowerCase().includes('hermes')) {
      checkedNS = true;
  ...
  self._enqueue('CAPABILITY', function() {
    ...
    if (self.state === 'connected') {
      ...
      var isServerSupportsID =this.serverSupports('ID');
      var cmd;
      if (self.serverSupports('AUTH=XOAUTH') && self._config.xoauth) {
        ...
        if (isServerSupportsID) {
          self._enqueue('ID ("version" "' + escape(self._idCommand.version) + '" "name" "' + escape(self._idCommand.name) + '" "vendor" "' + escape(self._idCommand.vendor) + '")', true);
        }
      } else {
